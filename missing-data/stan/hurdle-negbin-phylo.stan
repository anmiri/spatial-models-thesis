...
data {
  ...
  int ids_pap[ngr[5]];
  int ids_aus[ngr[6]];
  matrix[N, N] Lcov;                 // cholesky factor of phylo covariance matrix
}
...
parameters {
  ...
  // phylo
  vector[N] eta_phylo;                  // Vector of group effects
  real sigmaalpha;  // Vector of sd of group effect
  // for the hurdle
...
transformed parameters {
  ...
  // phylo
  vector[N] alpha = Lcov * eta_phylo * sigmaalpha;

  // GP basis stuff
  // 2 is the number of dimensions
  mu[ids_afr] = Intercept + mu_area[1] * sigma_intercept + alpha[ids_afr]; 
  mu[ids_eur] = Intercept + mu_area[2] * sigma_intercept + alpha[ids_eur]; 
  mu[ids_nam] = Intercept + mu_area[3] * sigma_intercept + alpha[ids_nam]; 
  mu[ids_sam] = Intercept + mu_area[4] * sigma_intercept + alpha[ids_sam]; 
  mu[ids_pap] = Intercept + mu_area[5] * sigma_intercept + alpha[ids_pap]; 
  mu[ids_aus] = Intercept + mu_area[6] * sigma_intercept + alpha[ids_aus];

  // hurdle mu

  hurdle_mu[ids_afr] = hurdle_intercept + hurdle_mu_area[1] * hurdle_sigma_intercept + alpha[ids_afr]; 
  hurdle_mu[ids_eur] = hurdle_intercept + hurdle_mu_area[2] * hurdle_sigma_intercept + alpha[ids_eur]; 
  hurdle_mu[ids_nam] = hurdle_intercept + hurdle_mu_area[3] * hurdle_sigma_intercept + alpha[ids_nam]; 
  hurdle_mu[ids_sam] = hurdle_intercept + hurdle_mu_area[4] * hurdle_sigma_intercept + alpha[ids_sam]; 
  hurdle_mu[ids_pap] = hurdle_intercept + hurdle_mu_area[5] * hurdle_sigma_intercept + alpha[ids_pap]; 
  hurdle_mu[ids_aus] = hurdle_intercept + hurdle_mu_area[6] * hurdle_sigma_intercept + alpha[ids_aus];
...
model {
  ...
  // phylo
  sigmaalpha ~ normal(1, 0.1);
  eta_phylo ~ std_normal();
