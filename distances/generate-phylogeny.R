...
build_phylos <- function(.lfd, .var, .micro_family = FALSE
  ...
  if (distance) {
    ...
    # transform distances to exponentially distributed branch lengths
    tree_dist <- as.dist(1/df.philo_d)
    hc <- hclust(tree_dist)
    tree <- as.phylo(hc)
    
    # generate exponential branch lengths (lambda = 1 for standard exponential)
    n_branches <- length(tree$edge.length)
    exp_lengths <- rexp(n_branches, rate = 1)
    
    # scale to reasonable range (optional can be adjusted)
    exp_lengths <- exp_lengths * mean(tree$edge.length) / mean(exp_lengths)
    
    tree$edge.length <- exp_lengths
    tree
  }
}
...
families <- read_csv("./data/languoid51.csv")
...
dsf$id2 <- make.unique(dsf$code_glot)

lfd <- filter(lang_fam_gloto_data, id %in% unique(dsf$code_glot))
afr_phylo <- build_phylos(lfd, id, .micro_family = FALSE)

lfd

write_rds(afr_phylo, "./data/phylogeny_exp_bl.rds")
